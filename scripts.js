...
// === Linear Regression Drawing Game ===
...
if (regressionCanvas && regressionResult && resetBtn) {
    ...
    regressionCanvas.addEventListener('mousedown', e => {
    ...
    regressionCanvas.addEventListener('mouseup', e => {
        const rect = regressionCanvas.getBoundingClientRect();
        const x = e.clientX - rect.left;
        const y = e.clientY - rect.top;
        userLine.push({ x, y });
...
canvas.addEventListener('mousedown', e => {
    ...
    freeformPath.moveTo(e.clientX - rect.left, e.clientY - rect.top);
    isDrawing = true;
});
...
canvas.addEventListener('mousemove', e => {
    if (!isDrawing) return;
    const rect = canvas.getBoundingClientRect();
    freeformPath.lineTo(e.clientX - rect.left, e.clientY - rect.top);
